...
def test_project_writer_requires_pinned_status_and_dispatch_field_ids(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("ADK_AGENTS_GITHUB_PRIMARY_SPECIALIST_FIELD_ID", "primary-field")
    monkeypatch.setenv("ADK_AGENTS_GITHUB_DISPATCH_FIELD_ID", "dispatch-field")
    monkeypatch.setenv("ADK_AGENTS_GITHUB_AGENT_SUMMARY_FIELD_ID", "summary-field")

    assert ServiceConfig.from_environment().project_writer_fields() == ("status-field", "dispatch-field", "summary-field")
